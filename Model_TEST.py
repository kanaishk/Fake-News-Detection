...
df_mc = pd.read_csv(os.path.join(dataset_dir,'train_news_preprocessed_mc.csv'), low_memory=False, 
                 usecols = ['label','clean_news_tokens'])
...
X_valid_mc = df_mc['clean_news_tokens'].apply(lambda x: ' '.join(eval(x))) 
y_valid_mc = df_mc['label']
...
X_valid_tfidf_mc = vectorizer.transform(X_valid_mc).copy()

modelname = 'Voting'
print('Model Training')

with open(os.path.join(model_dir,'trained_tuned_model.pkl'), 'rb') as f:
...
print(report)
with open(os.path.join(result_dir, f'{modelname}_Valid_report_mc.txt'), 'w') as f:
    f.write(report)
